...
import { fmtINRMoney, fmtINRDrCr, formatDateDMY, formatDateTimeDMY } from "@/lib/utils";
import { RiskLegendPopover } from "@/components/RiskLegendPopover";
import { ActivityLegendPopover } from "@/components/ActivityLegendPopover";
import { SaleTypeMultiSelect } from "@/components/SaleTypeMultiSelect";
import { SalesPersonMultiSelect } from "@/components/SalesPersonMultiSelect";
import { RiskMultiSelect } from "@/components/RiskMultiSelect";
...
export default function Dashboard() {
  ...
  const kpiCards = kpis ? [
    { label: totalCountLabel,      value: totalCountValue,                icon: Users,         warn: false, link: buildRRUrl(riskLevels.length > 0 ? `/dashboard/risk-register?risk=${riskLevels.join(",")}` : "/dashboard/risk-register"), toggle: null },
    ...
    { label: criticalLabel,        value: criticalValue,                  icon: ShieldAlert,   warn: true,  link: buildRRUrl("/dashboard/risk-register?risk=critical"),                   toggle: null },
    { label: "Over Credit Limit",  value: overLimitValue,                 icon: AlertTriangle, warn: true,  link: buildRRUrl("/dashboard/risk-register?filter=over_credit_limit"),         toggle: null },
    { label: "180+ Overdue",       value: overdue180Value,                icon: Users,         warn: true,  link: buildRRUrl("/dashboard/risk-register?aging=180%2B"),                    toggle: null },
    { label: blockedLabel,         value: blockedValue,                   icon: ShieldAlert,   warn: true,  link: buildRRUrl("/dashboard/risk-register?blocked=1"),                       toggle: null },
  ] : [];
              ...
              <BarChart data={aging}>
                ...
                <Bar
                  ...
                  onClick={(_: unknown, index: number) =>
                    navigate(buildRRUrl(`/dashboard/risk-register?aging=${encodeURIComponent(aging[index]?.bucket ?? "")}`))
                  }
                />
              </BarChart>
            </ResponsiveContainer>
            ...
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie
                  ...
                  cursor="pointer"
                  onClick={(_: unknown, index: number) =>
                    navigate(buildRRUrl(`/dashboard/risk-register?risk=${displayRiskSegmentation[index]?.name.toLowerCase() ?? ""}`))
                  }
                >
        ...
        <Card className="rounded-card lg:col-span-2">
          <CardHeader className="pb-2 flex flex-row items-center justify-between">
            ...
            <Button
              ...
              onClick={() => navigate(buildRRUrl("/dashboard/risk-register?risk=critical,high"))}
            >
              ...
              {displayTopRisky.length === 0 ? (
                ...
                displayTopRisky.map((c) => {
                  ...
                  const target = isGroupMode && c.isGroup
                    ? `/dashboard/group/${encodeURIComponent(c.name)}`
                    : `/dashboard/customer/${encodeURIComponent(c.name)}`;
                  return (
                    <div
                      key={c.id}
