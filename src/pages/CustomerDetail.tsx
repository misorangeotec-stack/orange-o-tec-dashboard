...
import { fmtINRMoney, fmtINRDrCr, formatDateDMY } from "@/lib/utils";
...
export default function CustomerDetail() {
  ...
  //   /dashboard/customer/:id  → :id holds encodeURIComponent(customerName)
  //   /dashboard/group/:id     → :id holds encodeURIComponent(groupName)
  const { id: nameEncoded } = useParams<{ id: string }>();
  const decoded = decodeURIComponent(nameEncoded ?? "");
  const location = useLocation();
  const isGroupRoute = location.pathname.includes("/dashboard/group/");
    ...
    <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
        ...
        <div className="space-y-2">
          <Button
            ...
            onClick={() => {
              ...
              navigate(qs ? `/dashboard/risk-register?${qs}` : "/dashboard/risk-register");
            }}
      ...
      {customer.overdue > 0 && (() => {
        ...
        const agingData = AGING_BUCKETS.map(({ label, key, color }) => ({
          label,
          color,
          amount: (customer.agingBuckets as Record<string, number>)?.[key] ?? 0,
        })).filter((d) => d.amount > 0);
                  ...
                  {agingData.map((d) => {
                    ...
                    return (
                      <div
                        ...
                        style={isActive ? { backgroundColor: `${d.color}18`, ringColor: d.color } : {}}
                      >
