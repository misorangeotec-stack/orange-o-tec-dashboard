...
import type { AgingBuckets, ConsolidatedCustomer, GroupedCustomer, ProposedCreditLimitReason, ProposedConstituent } from "@/lib/types";
              ...
              {paginatedRows.length === 0 ? (
                ...
                paginatedRows.flatMap((row) => {
                  ...
                  const navTarget = isExpandableGroup
                    ? `/dashboard/group/${encodeURIComponent(row.name)}`
                    : `/dashboard/customer/${encodeURIComponent(row.name)}`;
                  ...
                  if (isExpanded) {
                    for (const child of visibleChildren) {
                      out.push(renderRow(child, {
                        key: `${row.id}::${child.id}`,
                        isChild: true,
                        onClick: () => navigate(`/dashboard/customer/${encodeURIComponent(child.name)}`),
                        leadCell: (
